...
impl ChannelDb {
    ...
    /// Finds all channels matching the given origin.
    pub fn find_by_origin(&self, origin: &str) -> Result<Vec<Channel>, StoreError> {
        ...
        for row in rows {
            channels.push(row.map_err(|e| StoreError::Internal(e.to_string()))?);
        }
        Ok(channels)
    }
    ...
    /// Inserts or updates a channel. On conflict, `created_at` is preserved.
    ...
    /// Deletes a channel by ID. Returns `true` if a row was removed.
